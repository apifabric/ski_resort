...
resourcename="skiresort"  # lower case, only
resourcegroup="skiresort_rg"
...
# git clone https://github.com/apilogicserver/skiresort.git
# cd skiresort
...
az group create --name skiresort_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group skiresort_rg --sku S1 --is-linux
az container create --resource-group skiresort_rg --name skiresort --image apilogicserver/skiresort:latest --dns-name-label skiresort --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//skiresort.westus.azurecontainer.io:5656'
